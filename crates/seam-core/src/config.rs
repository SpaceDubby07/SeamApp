...
//! milestone owns (paired-peer cert fingerprints) rather than guessing
//! their shape now.
...
pub struct Config {
    ...
    /// pairing, which keys on it.
    pub node_id: NodeId,
    /// User-facing name shown to peers, e.g. "Zach's laptop".
    pub display_name: String,
    /// Hard cap, in bytes, on clipboard content we'll sync to the peer.
    /// Content over this is skipped entirely — not truncated — and
    /// logged. Defaults to 10 MB. `#[serde(default)]` so a config file
    /// written before this field existed still loads.
    #[serde(default = "default_clipboard_max_bytes")]
    pub clipboard_max_bytes: u64,
    /// The one peer we've paired with — a single `Option` rather than a
    /// collection, since v1 pairs exactly one peer. `#[serde(default)]`
    /// so a config file written before pairing existed still loads, as
    /// "not yet paired with anyone."
    #[serde(default)]
    pub paired_peer: Option<PairedPeer>,
    /// Whether to prompt, always accept, or always deny incoming file
    /// transfer offers from the paired peer. `#[serde(default)]` so a
    /// config file written before this existed still loads, defaulting
    /// to `AcceptPolicy::Ask`.
    #[serde(default)]
    ...
    /// later OS-level change. `#[serde(default)]` for the same
    /// forward-compat reason as `accept_policy`.
    #[serde(default)]
...
impl Config {
    /// A fresh default config: a new random node identity, a
    /// hostname-derived display name, the default 10 MB clipboard cap,
...
/// can always rename in the Connection panel.
...
/// 10 MB — the default clipboard sync size cap.
...
mod tests {
    ...
    /// An older config predating the clipboard cap must not fail to load —
    /// it should fall back to the default cap via `#[serde(default)]`.
    #[test]
    fn config_written_before_clipboard_cap_existed_still_loads() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("config.toml");
        let pre_cap_toml = format!(
            "node_id = \"{}\"\ndisplay_name = \"old machine\"\n",
            uuid::Uuid::new_v4()
        );
        std::fs::write(&path, pre_cap_toml).expect("write pre-cap config");

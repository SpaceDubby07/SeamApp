//! Diagnostic demo: measures RAW captured mouse-delta magnitude on Windows,
//! unsuppressed vs suppressed, with zero `seam-core::Session`/network code
//! in the loop — isolating whether a real physical sweep of the mouse
//! produces large `MouseDelta`s at the capture layer itself, independent
//! of everything the session/state machine does with them afterward.
//!
//! Why this exists: two-machine testing showed the driven side's cursor
//! barely moving (~50px net over ~1.5s of driving) despite the user
//! reporting they were moving the physical mouse a lot. Rather than
//! keep patching capture.rs blind, this isolates the one variable that
//! matters — does suppression itself change what the low-level hook
//! reports? — before touching anything else.
//!
//! Run on Windows with:
//!   cargo run -p seam-platform --example windows_suppression_delta_demo
//!
...
//!   happens while `RemoteActive` in the real app.
//!
//! Each phase prints a running total and, at the end, a summary: sample
//! count, sum of |dx| (total raw travel regardless of direction), and net
//! dx (final accumulated position — what `Session::driving_cursor` would
//! see). If phase 2's sum-of-|dx| is far below phase 1's for the same
//! physical effort, suppression itself is eating motion. If BOTH are
//! small, the problem isn't suppression-specific — it's capture itself
//! (or the physical input device/OS pointer settings).
//!
...
fn main() {
    ...
    use seam_platform::windows::Capture;
    ...
    if samples == 0 {
        ...
        println!(
            "  -> total travel is tiny even summed across the whole phase — suspect capture \
             itself (OS pointer settings, or the hook), not anything downstream."
        );
    }
}
...
#[cfg(not(windows))]
fn main() {
    println!("windows_suppression_delta_demo is Windows-only; nothing to run on this platform.");
}
